...
function checkin(){
  openTodayModal('<h2 class="modal-title">How are you feeling today?</h2><p class="modal-sub">Add your energy and soreness so FIT-OS can use your human signal alongside training data.</p><div class="check-options" id="moods"><button type="button" data-v="Great"><small>ENERGY</small><b>Great</b></button><button type="button" data-v="Good" class="selected"><small>ENERGY</small><b>Good</b></button><button type="button" data-v="Okay"><small>ENERGY</small><b>Okay</b></button><button type="button" data-v="Low"><small>ENERGY</small><b>Low</b></button></div><div class="check-options" id="soreness"><button type="button" data-v="None" class="selected"><small>SORENESS</small><b>None</b></button><button type="button" data-v="Mild"><small>SORENESS</small><b>Mild</b></button><button type="button" data-v="Moderate"><small>SORENESS</small><b>Moderate</b></button><button type="button" data-v="High"><small>SORENESS</small><b>High</b></button></div><button type="button" class="modal-submit" id="saveCheckin">Save check-in</button>');
...
function openExercise(index){
  ...
  body.innerHTML=`<div class="eyebrow">${exercise.type} · EXERCISE ${String(index+1).padStart(2,"0")}</div><h2>${exercise.name}</h2><p class="description">${exercise.desc}</p>${setContent}<button type="button" class="exercise-main-action" id="completeExercise">${index===exercises.length-1?"Complete session":"Complete & continue"} ›</button><div class="rest-panel"><small>REST TIMER</small><div class="rest-time" id="restTime">${String(Math.floor(exercise.rest/60)).padStart(2,"0")}:${String(exercise.rest%60).padStart(2,"0")}</div><div class="rest-controls"><button type="button" id="restStart">Start</button><button type="button" id="restPlus">+30s</button><button type="button" id="restStop">Stop</button></div></div>`;
